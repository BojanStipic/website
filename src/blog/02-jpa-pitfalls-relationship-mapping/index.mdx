...
In this article we are going to discuss some common relationship mapping pitfalls.
...
```java
Set<Comment> findByPost_id(Long postId, Pageable pageable);
```
...
We could now run custom queries to fetch what we need:
...
And of course, we could now pass in a `Pageable` instance to get a pageable response.
...
The only exception to this rule is if you know that there will only ever exist very few
associated entities.
...
These common mistakes, that create huge performance problems,
could probably be avoided if `@OneToMany` was actually named `@OneToFew`
to suggest that it should only be used when there are _few_ associated entities.
But it is too late now, so we can only try and educate people to stop using these
