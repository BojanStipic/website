...
In this article we are going to discuss about different JPA strategies
for generating IDs.
...
- [Generating IDs](/blog/04-jpa-pitfalls-generating-ids/) (this article)
- JPA and Lombok (TBD)
...
Unfortunately, if we don't care about implementation details we can end up
...
`strategy` attribute of `@GeneratedValue` annotation.
For example:
...
different strategies imply, and see which one should you prefer.
...
However, they are now standardised and you can use the following syntax instead:

```sql
create table t (id bigint primary key generated always as identity);
```

In Postgres, serial types are nothing special -- they are just a syntax sugar and use the
...
Hibernate can now create insert statements without specifying ID column
...
the JPA provider still needs to fetch that ID afterwords in order to know what the generated value is.
...
with the JPA entities afterwords,
...
So far we discussed only about database generated IDs.
...
On top of that, Hibernate also provides an implementation for UUID generator,
...
- UUIDs are 128bit, which is double the size of a `bigint`, or four times
  ...
  more frequent memory swapping which is bad for performance.
...
To summarise, here is a cheatsheet for choosing the best strategy (from worst to best):
